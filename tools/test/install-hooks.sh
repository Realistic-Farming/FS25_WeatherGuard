#!/usr/bin/env bash
# Install this repo's git pre-commit hook (Lua 5.1 syntax, plus lint where present).
...
echo "Runs the syntax (and lint, if present) check when .lua files are staged. Skip once with: git commit --no-verify"
